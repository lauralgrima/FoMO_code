            ...
            for a4 = a4_vec
                for a5 = a5_vec
                    ...
                    parfor iter = 1:num_iter
                        [trans_r2_iter(1,iter),income_r2_iter(1,iter), vismat(:,:,iter),~] = HX_model_session_forAlphaOpt(a1,a2,a3,a4,a5,alpha_version,visit_matrix,cost_per_port,rew_sched,income,prior);
                    end
                    ...
                    % figure(10); subplot(1,numel(a2_vec),find(a2==a2_vec)); plot(v_ind,alpha_vis); hold on; axis([v_ind(1) v_ind(end) 0 max(a2_vec).^2]); box off;
                    if find(a5==a5_vec)==numel(a5_vec)
                        figure(11); subplot(3,numel(a2_vec),find(a2==a2_vec));
                        imagesc(squeeze(opt_r2_tensor(find(a2==a2_vec),:,:)),[0.55 0.95]); colormap(exag_map);
                        title('Trans R2');
                        figure(11); subplot(3,numel(a2_vec),find(a2==a2_vec)+numel(a2_vec));
...
%% Convert this to a nice function
v_ind = 1:1000;
x2 = 1;
x3 = 1;
x1 = 0;
x5 = 500;
x4 = 70;
alpha_vis = (x2 ./ (1+exp((x4-v_ind)/(x4./2.67)))) .*  (x3*exp(-v_ind/x5));
figure(9); clf; plot(alpha_vis);

x = [1:numel(hexa_data_an.da_resp_all.r)]';
y = hexa_data_an.da_resp_all.r;
[f,gof] = fit(x(1:end),y(1:end),'exp1');
figure(9); clf; plot(f,x,y);
...
for zz=1:numel(all_sess_files)
    ...
    all_r2      = [all_r2 ; max(opt_r2_tensor,[],"all")];
    ...
    % compare to decay tau of exponential fit to dopamine

    % load the corresponding hexa_data_an struct
        load([all_sess_files(zz).name(1:end-7) 'an.mat']);
...
targety = hexa_data_an.da_resp_all.r./max(movmean(hexa_data_an.da_resp_all.r,21))

a0 = [ 0 0.5 0.5 100 500 ]
[fitted_curve,gof] = fit([1:numel(hexa_data_an.da_resp_all.r)]',targety,fitfun,'StartPoint',a0,'Upper',[1 1 1 500 1000],'Lower',[0 0 0 0 0]);
fitted_curve
figure(900); clf;
scatter([1:numel(hexa_data_an.da_resp_all.r)]',targety, 'r+')
hold on
plot([1:numel(hexa_data_an.da_resp_all.r)]',movmean(targety,21), 'r-')
plot([1:numel(hexa_data_an.da_resp_all.r)]',fitted_curve([1:numel(hexa_data_an.da_resp_all.r)]'));
hold off
% 
% % Solve it
