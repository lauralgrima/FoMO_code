...
for mmm = 1:numel(all_files)
    ...
    for reps=1:10
        ...
        [hexa_model]        = HX_model_session_2(hexa_data_an,policy_model,belief_model,hexa_model.cost_per_port,1,0);    
        [port,event_time]   = find(hexa_model.visits==1);
        rewarded            = sum(hexa_model.rewards(:,event_time),1)';
        T                   = table(port,rewarded,event_time);
        writetable(T, ['ModelTestExport_' mouse_name '_' policy_model '_' belief_model '_' num2str(reps) '.csv']);
...
if run_new
    % mmm = randperm(numel(all_files),1)
