...
vis_mean_OS = movmean(squeeze(mean(vismat_OS,3)),win_run,2)+0.0001;
vis_mean_OSE = movmean(squeeze(mean(vismat_OSE,3)),win_run,2)+0.0001;

rew_mean = movmean(squeeze(mean(rewmat,3)),win_run,2)+0.0001;
rew_mean_OS = movmean(squeeze(mean(rewmat_OS,3)),win_run,2)+0.0001;
rew_mean_OSE = movmean(squeeze(mean(rewmat_OSE,3)),win_run,2)+0.0001;
...
log10_choiceOS = zeros(size(vis_mean));
log10_rewardOS = zeros(size(vis_mean));
...
for pp=1:6
    ...
    log10_choiceOS(pp,:) = log10(vis_mean_OS(pp,:) ./ (sum(vis_mean_OS,1)-vis_mean_OS(pp,:)));
    subplot(121);
    plot(log10_choice(pp,:),'color',sess_map(pp,:)); hold on;
    plot(log10_choiceOS(pp,:),'color',sess_map(pp,:),'Linewidth',2); hold on;
    xlim([0.8e4 2e4]);
    ...
    log10_rewardOS(pp,:) = log10(rew_mean_OS(pp,:) ./ (sum(rew_mean_OS,1)-rew_mean_OS(pp,:)));


    log10_choiceD(pp,:) = log10(vis_mean_d(pp,:) ./ (sum(vis_mean_d,1)-vis_mean_d(pp,:)));
    log10_rewardD(pp,:) = log10(rew_mean_d(pp,:) ./ (sum(rew_mean_d,1)-rew_mean_d(pp,:)));

    % plot(log10_reward(pp,:),'color',sess_map(pp,:)); hold on;
    % plot(log10_rewardOS(pp,:),'color',sess_map(pp,:),'Linewidth',2); hold on;
    % xlim([0.8e4 1.5e4]);
end
...
% xlim([0e4 2e4]); title('Port 1');  ylabel('Choice rate');
% 
% subplot(322);
...
% xlim([0e4 2e4]);  ylabel('Reward rate'); legend({'Cntrl' 'Alpha boost' 'Just error increase'});
% 
% subplot (323);
...
% xlim([0e4 2e4]); title('Port 4');
% 
% subplot(324);
% plot(movmean(rew_mean(4,:),win_run)); 
% hold on;
% plot(movmean(rew_mean_OS(4,:),win_run)); 
% plot(movmean(rew_mean_OSE(4,:),win_run)); 
% xlim([0e4 2e4]);
% 
% subplot (325);
...
% xlim([0e4 2e4]); title('Port 6');
% 
% subplot(326);
% plot(movmean(rew_mean(6,:),win_run)); 
% hold on;
% plot(movmean(rew_mean_OS(6,:),win_run)); 
% plot(movmean(rew_mean_OSE(6,:),win_run)); 
% xlim([0e4 2e4]);
