...
num_mc = 200;
for kk=1:num_mc
    shift_x = randperm(numel(hexa_data_an.da_resp_all.r));
    circ_shift = hexa_data_an.da_resp_all.r(shift_x);  
    tmp_da_resp_mag = conv( circ_shift , trial_kernel , 'same' );
...
seq_map = TNC_CreateRBColormap(1e3,'cpb');
...
figure(101); clf;
for qq=1:size(trans_p_dist.base_p,1)
    plot(1:6,trans_p_dist.base_p(qq,:),'color',cat_map(qq,:),'LineWidth',2); hold on;
end
box off; axis([1.9 6.1 0 0.5]); hold on;
plot(1:6,trans_mat_data(:,1)./sum(trans_mat_data(:,1)),'color',cat_map(qq+1,:),'LineWidth',2);

figure(102); clf;
port1_inds = find(hexa_data_an.da_resp_all.p==1);
boxplot(hexa_data_an.da_resp_all.r(port1_inds)',hexa_data_an.da_resp_all.pp(port1_inds)');
...
% hold on;
% port1_inds = find(hexa_data_an.da_resp_all.pu==1);
% boxplot(hexa_data_an.da_resp_all.u(port1_inds)',hexa_data_an.da_resp_all.ppu(port1_inds)','color','k');
% scatter(hexa_data_an.da_resp_all.t(port1_inds),hexa_data_an.da_resp_all.r(port1_inds),50,hexa_data_an.da_resp_all.pp(port1_inds),'filled'); colormap(cat_map);
% swarmchart(hexa_data_an.da_resp_all.pp(port1_inds),hexa_data_an.da_resp_all.r(port1_inds),50,log(hexa_data_an.da_resp_all.tl(port1_inds)),'filled'); colormap(seq_map);
