function [hexa_data_an] = HX_analyze_session(hexa_data,session,photo)
...
%     port_color_map      = [port_color_map(1,:) ; port_color_map(3:8,:) ];

    sym_map             = TNC_CreateRBColormap(1000,'bb-sym');
    
    photo_event_win     = [150 500];
...
%     visit_indices       = find(hexa_data.port_n>0 & ~isnan(hexa_data.photo_i) & hexa_data.session_n==session);
    ...
    for pp=1:6
        ...
        subplot(1,6,pp);
        plot([0 find(rew_visit_ids==1,1,'last')],[0 0],'k-');  hold on;
        pN_inds = find( port_visit_ids==pp & rew_visit_ids==0 );
        scatter(pN_inds,trapz(sink.wins(pN_inds,photo_event_win(1)-75:photo_event_win(1)+325),2),50,port_color_map(pp,:)); colormap(port_color_map);
        pR_inds = find( port_visit_ids==pp & rew_visit_ids==1 );
        scatter(pR_inds,trapz(sink.wins(pR_inds,photo_event_win(1)-75:photo_event_win(1)+325),2),50,port_color_map(pp,:),"filled"); colormap(port_color_map);
        axis([0 find(rew_visit_ids==1,1,'last') -500 1500]);
        box off; grid on; title(['Port n=' num2str(pp)]);
        if pp==1
            ylabel('Integrated DA resp.'); xlabel('Unique visits'); 
        end
        yyaxis right;
        plot(sgolayfilt(p_rew_this_port',3,21)); axis([0 find(rew_visit_ids==1,1,'last') 0 1]);
        if pp==1
            ylabel('P(rew|visit)');
        end
    end
