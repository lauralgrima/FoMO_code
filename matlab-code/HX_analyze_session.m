...
%     visit_indices       = find(hexa_data.port_n>0 & ~isnan(hexa_data.photo_i) & hexa_data.session_n==session);
    ...
    for pp=1:6
        ...
        scatter(hexa_data.event_time(visit_indices(pR_inds)),trapz(sink.wins(pR_inds,photo_event_win(1):photo_event_win(1)+325),2),50,port_color_map(pp,:),"filled"); colormap(port_color_map);
