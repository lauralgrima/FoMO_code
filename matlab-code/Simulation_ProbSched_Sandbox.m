...
%% SIMULATING IF DECAYING DA SIGNAL (ALPHA) WAS THE ERROR...
...
for opto_on = [0 -1 2 4]
    ...
    for mice = 1:25
        ...
        for vi = 2:numel(visit_index)
            ...
            % update chosen option
            p_reward(checked_port(vi),vi)   = p_reward(checked_port(vi),vi-1) + (yes_reward(vi))*this_alpha;
            p_stay(checked_port(vi),vi)     = p_stay(checked_port(vi),vi-1)   + (yes_reward(vi))*this_alpha;
...
figure(1); clf; 
subplot(211); boxchart(final_boxplot(:,1:2));
xticklabels({'Control' 'Stim 3-6'}); ylim([-1 1]); ylabel('sensitivity');
subplot(212); boxchart(final_boxplot(:,3:4));
xticklabels({'Switch Control' 'Switch Stim All'}); ylim([-1 1]); ylabel('sensitivity');